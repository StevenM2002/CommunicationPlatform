...
def setup_public():
    ...
    r = requests.post(
        ...
        json={"token": token, "name": "public_channel_bud", "is_public": True},
    )
    assert r.status_code == 200
...
def test_admin_user_remove(setup_public):
    ...
    r = requests.post(
        f"{config.url}channels/create/v2",
        json={"token": user_token, "name": "pub_channel", "is_public": True},
    )
    assert r.status_code == 200
    c_id = r.json()["channel_id"]
    r = requests.post(
        f"{config.url}dm/create/v1", json={"token": token, "u_ids": [u_id]}
    )
    assert r.status_code == 200
    dm_id = r.json()["dm_id"]
    r = requests.post(
        f"{config.url}dm/create/v1", json={"token": user_token, "u_ids": [token_id]}
    )
    assert r.status_code == 200
    r = requests.post(
        f"{config.url}dm/create/v1", json={"token": token, "u_ids": [throwaway]}
    )
    r = requests.post(
    ...
    assert r.status_code == 200
    r = requests.delete(
        f"{config.url}admin/user/remove/v1", json={"token": token, "u_id": u_id}
    )
    assert r.status_code == 200
    r = requests.get(
    ...
    r = requests.get(f"{config.url}users/all/v1", params={"token": token})
    assert r.status_code == 200
    users = r.json()["users"]
    for user in users:
        assert user["u_id"] != u_id
    r = requests.get(
        f"{config.url}user/profile/v1", params={"token": token, "u_id": u_id}
    )
    assert r.status_code == 200
    user = r.json()["user"]
    assert user["name_first"] == "Removed" and user["name_last"] == "user"
    r = requests.get(
        f"{config.url}dm/details/v1", params={"token": token, "dm_id": dm_id}
    )
    assert r.status_code == 200
...
def test_admin_userpermission_change(setup_public):
    ...
    r = requests.post(
        ...
        json={"token": token, "u_id": u_id, "permission_id": 2},
    )
    assert r.status_code == 200
    r = requests.post(
        f"{config.url}admin/userpermission/change/v1",
        json={"token": token, "u_id": u_id, "permission_id": 1},
    )
    r = requests.post(
        f"{config.url}channels/create/v2",
        json={"token": token, "name": "public_channel_2", "is_public": False},
    )
    assert r.status_code == 200
    r = requests.post(
        f"{config.url}channel/join/v2",
        json={"token": user_token, "channel_id": r.json()["channel_id"]},
    )
    assert r.status_code == 200
    r = requests.post(
        f"{config.url}admin/userpermission/change/v1",
    ...
    r = requests.post(
        f"{config.url}channels/create/v2",
        json={"token": token, "name": "public_channel_3", "is_public": False},
    )
    assert r.status_code == 200
    r = requests.post(
        f"{config.url}channel/join/v2",
...
def test_userpermission_perm_id_invalid(setup_public):
    ...
    r = requests.post(
        f"{config.url}admin/userpermission/change/v1",
        json={"token": token, "u_id": u_id, "permission_id": 3},
    )
    assert r.status_code == InputError.code
